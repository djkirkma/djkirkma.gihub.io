...
d3.csv("https://raw.githubusercontent.com/djkirkma/testrepo/main/games.csv?token=AKTDRCFVNZ5J36ITUUPRNPLBXESC6", function (error, data) {
  ...
  function doAMove(target) {
    ...
    NewList3 = {}
    OpeningList1 = []
    RatedOrUnrated = document.getElementById("RatedOrUnrated")
    ratedVal = RatedOrUnrated.options[RatedOrUnrated.selectedIndex].value
    ...
    for (x = 0; x < MaxCount; x++) {
      if (OUtput) {
        ...
        if (data[x].moves.substring(0, theMovesPlayed.length) == theMovesPlayed && data[x].rated == "TRUE") {
          console.log(data[x].rated)
          average = (parseInt(data[x].white_rating) + parseInt(data[x].black_rating)) / 2
          Rounded = Math.ceil(average / 100) * 100
          holder = data[x].moves
          //TheNextMove = data[x].moves.substring(theMovesPlayed,theMovesPlayed+2)
          openName = data[x].opening_name
          //console.log(openName)
          TheNextMove = holder.substring(theMovesPlayed.length, theMovesPlayed.length + 2)
          if (!(openName in OpeningList1)) {
            OpeningList1.push(openName)
          }
    ...
    openingVar = document.getElementById("openingName")
    openingVar.innerHTML = OpeningList1[0]
